...
#let bold-figure-caption(fig-cap, loc) = context {
  strong({
    fig-cap.supplement
    " "
    // Handle both string patterns and function-based numbering
    if type(fig-cap.numbering) == function {
      // For function-based numbering (e.g., subfloats), call with counter values
      // and ensure we get content, not a function
      let result = (fig-cap.numbering)(..fig-cap.counter.at(loc))
      if type(result) == function {
        // theorion-style returns a curried function; call it to get final content
        result()
      } else {
        result
      }
    } else if fig-cap.numbering != none {
